...
LIGAN O. F. Patrick ( +229 67 44 62 18)
 ...
 -->
    ...
    <meta charset="utf-8" />
    <title>Messages reçus (Boîte de réception) | APHRODITE CLUB</title>
													...
													<tr>
														<td>
															<form style="" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
																<input type="text" name="searchmembre" style="font-style:italic;width:260px;" placeholder="Recherchez un expéditeur" /> <input type="submit" name="search" value="OK" class="club-button" />
															</form>
														</td>
														<td>
															<form style="" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
																<input type="text" name="searchmessage" style="font-style:italic;width:260px;" size="58" placeholder="Recherchez un message" /> <input type="submit" name="search2" value="OK" class="club-button" />
															</form>
														</td>
													</tr>
												</table>
												...
												<br />
													...
													<table style="width: 100%; font-size:13px;">
														...
														<?php 
																...
																$req = $bdd->query('SELECT idmessages,titremessage,naturemessage,destinatairemessage,expediteurmessage,DATE_FORMAT(datemessages, "%d/%m/%Y à %Hh%imin") AS datemessagesfr FROM messages WHERE expediteurmessage = "'.$_POST["searchmembre"].'" AND destinatairemessage = "'.$_SESSION["pseudousers"].'" AND statutdestinactifs = "actif" ORDER BY idmessages DESC LIMIT 0,100 ');
														...
														?>
																		<tr>
																			<td style="font-size:15px;padding-left: 15px;"><span style="color:#ED1E02;"><a href="readmessage.php?idmessage=<?php echo $donnees["idmessages"];?>" title="Cliquer ici pour lire le message"><?php echo $donnees["titremessage"]; ?></a></span></td>
																			...
																			<td style="text-align:center;"><a href="deletemessage.php?iddelete=<?php echo $donnees["idmessages"];?>&pagesend=messagerecu.php" onclick="return confirm('Etes vous sûr de supprimer ce message ?');"><img src="images/delete-icone-supprimer.png" /></a></td>
																		</tr>
														<?php 
																	}
																	else
														...
														?>
																		<tr>
																			<td style="font-size:15px;padding-left: 15px;"><a href="readmessage.php?idmessage=<?php echo $donnees["idmessages"];?>" title="Cliquer ici pour lire le message" style="color:#000000;"><?php echo $donnees["titremessage"]; ?></a></td>
																			<td style="text-align:center;"><?php echo $donnees["expediteurmessage"]; ?></td>
																			<td style="font-style:italic;text-align:center;line-height: 45px;"><?php echo $donnees["datemessagesfr"]; ?></td>
																			<td style="text-align:center;"><a href="deletemessage.php?iddelete=<?php echo $donnees["idmessages"];?>&pagesend=messagerecu.php" onclick="return confirm('Etes vous sûr de supprimer ce message ?');"><img src="images/delete-icone-supprimer.png" /></a></td>
																		</tr>
													...
													<table style="width: 100%; font-size:13px;">
														...
														<?php 
																...
																$req = $bdd->query("SELECT idmessages,titremessage,naturemessage,destinatairemessage,expediteurmessage,contenumessage,DATE_FORMAT(datemessages, '%d/%m/%Y à %Hh%imin') AS datemessagesfr FROM messages WHERE titremessage LIKE '%".$_POST["searchmessage"]."%' AND  destinatairemessage = '".$_SESSION["pseudousers"]."' AND statutdestinactifs = 'actif' OR contenumessage LIKE '%".$_POST["searchmessage"]."%' AND  destinatairemessage = '".$_SESSION["pseudousers"]."' AND statutdestinactifs = 'actif' ORDER BY idmessages DESC LIMIT 0,100 ");
																while($donnees = $req->fetch())
																{
													...
													<table style="width: 100%; font-size:13px;">
														...
														<?php 
																...
																include 'paginationmessagerecu.php';
															   /* On calcule le nombre total d'entrées de notre table 
																09.que l'on stocke dans $nb_entrees */
																$req = $bdd->query("SELECT COUNT(*) AS total FROM messages WHERE destinatairemessage = '".$_SESSION["pseudousers"]."' AND statutdestinactifs = 'actif' ");
																...
																$req = $bdd->query('SELECT idmessages,titremessage,naturemessage,destinatairemessage,expediteurmessage,DATE_FORMAT(datemessages, "%d/%m/%Y à %Hh%imin") AS datemessagesfr FROM messages WHERE destinatairemessage = "'.$_SESSION["pseudousers"].'" AND statutdestinactifs = "actif" ORDER BY idmessages DESC LIMIT '.$start.', '.$entrees_par_page.' ');
																while($donnees = $req->fetch())
																{
