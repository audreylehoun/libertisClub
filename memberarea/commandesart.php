...
LIGAN O. F. Patrick ( +229 67 44 62 18)
 ...
 -->
    ...
    <meta charset="utf-8" />
    <title>Historique de vos commandes | LIBERTIS CLUB</title>
											...
											<div class="club-layout-cell layout-item-0" style="width: 100%;" >
													...
													<tr>
														<td style="padding:10px;text-align:center;">
															<form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
																...
																<option value="libelles"> Libellés </option>
																</select><br /><br />
												...
												<table style="margin-left:0px;width: 100%;">
													<tr>
														...
														<th style="text-align:center;padding:10px;font-size:15px;"><strong>Paiement</strong></th>
														<th style="text-align:center;padding:10px;font-size:15px;"><strong>Livraison</strong></th>
														<th style="text-align:center;padding:10px;font-size:15px;"><strong>Date d'achat</strong></th>
													</tr>
												<?php
												if(isset($_POST["datefiltre"]))
												{
													$req = $bdd->query('SELECT pseudo_panierachat, libelle_panierachat, prixunit_panierachat, qte_panierachat, montant_panierachat, statutachat_panierachat, statutlivrai_panierachat, DATE_FORMAT(dateachat_panierachat, "%d/%m/%Y") AS dateachat_panierachatfr FROM panierachat WHERE pseudotransaction = "'.$_SESSION["pseudousers"].'" AND dateachat_panierachat BETWEEN "'.$_POST["datedebut"].'" AND "'.$_POST["datefin"].'" ORDER BY id_panierachat DESC LIMIT 0,100 ');
													...
													{	
														...
														?>
															<tr>
																<td style="text-align:center;"><strong><em><?php echo $libel;?></em></strong></td>
																<td style="text-align:center;"><strong><?php echo $donnees["montant_panierachat"];?></strong></td>
																<td style="text-align:center;">
																<?php
																if($donnees["statutachat_panierachat"] == "non paye") 
																{
																?>
																	<span style="color: rgb(255, 0, 0);"><strong><?php echo $donnees["statutachat_panierachat"];?> </strong></span>
																<?php
																}
																else 
																{
																?>
																	<span style="color: rgb(50, 178, 50);"><strong><?php echo $donnees["statutachat_panierachat"];?> </strong></span>
																<?php
																}
																?>
																</td>
																<td style="text-align:center;">
																<?php
																if($donnees["statutlivrai_panierachat"] == "non livre") 
																{
																?>
																	<span style="color: rgb(255, 0, 0);"><strong><?php echo $donnees["statutlivrai_panierachat"];?> </strong></span>
																<?php
																}
																elseif($donnees["statutlivrai_panierachat"] == "En cours") 
																{
																?>
																	<span style="color:orange;"><strong><?php echo $donnees["statutlivrai_panierachat"];?> </strong></span>
																<?php
																}
																else 
																{
																?>
																	<span style="color: rgb(50, 178, 50);"><strong><?php echo $donnees["statutlivrai_panierachat"];?> </strong></span>
																<?php
																}
																?>
																</td>
																<td style="text-align:center;"><em><?php echo $donnees["dateachat_panierachatfr"];?></em></td>
															</tr>
														<?php
													}
													$req->closeCursor();
												...
												{
													$req = $bdd->query('SELECT pseudo_panierachat, libelle_panierachat, prixunit_panierachat, qte_panierachat, montant_panierachat, statutachat_panierachat, statutlivrai_panierachat, DATE_FORMAT(dateachat_panierachat, "%d/%m/%Y") AS dateachat_panierachatfr FROM panierachat WHERE pseudo_panierachat = "'.$_SESSION["pseudousers"].'" AND  montant_panierachat = "'.$_POST["montant"].'" OR pseudo_panierachat = "'.$_SESSION["pseudousers"].'" AND prixunit_panierachat = "'.$_POST["montant"].'" ORDER BY id_panierachat DESC LIMIT 0,100 ');
													while($donnees = $req->fetch())
													{	
														$libel = "<span style='font-size:15px;color: red;'><strong>".$donnees["qte_panierachat"]."</strong></span> ".$donnees["libelle_panierachat"]." de <strong>".$donnees["prixunit_panierachat"]."</strong> F ";	
														?>
												...
												elseif(isset($_POST["searchfirst"]) AND $_POST["searchin"] == "paiement")
												{
													$req = $bdd->query('SELECT pseudo_panierachat, libelle_panierachat, prixunit_panierachat, qte_panierachat, montant_panierachat, statutachat_panierachat, statutlivrai_panierachat, DATE_FORMAT(dateachat_panierachat, "%d/%m/%Y") AS dateachat_panierachatfr FROM panierachat WHERE pseudo_panierachat = "'.$_SESSION["pseudousers"].'" AND statutachat_panierachat LIKE "%'.$_POST["searchmot"].'%" ORDER BY id_panierachat DESC LIMIT 0,100 ');
													while($donnees = $req->fetch())
													{	
														$libel = "<span style='font-size:15px;color: red;'><strong>".$donnees["qte_panierachat"]."</strong></span> ".$donnees["libelle_panierachat"]." de <strong>".$donnees["prixunit_panierachat"]."</strong> F ";	
														?>
												...
												elseif(isset($_POST["searchfirst"]) AND $_POST["searchin"] == "livraison")
												{
													$req = $bdd->query('SELECT pseudo_panierachat, libelle_panierachat, prixunit_panierachat, qte_panierachat, montant_panierachat, statutachat_panierachat, statutlivrai_panierachat, DATE_FORMAT(dateachat_panierachat, "%d/%m/%Y") AS dateachat_panierachatfr FROM panierachat WHERE pseudo_panierachat = "'.$_SESSION["pseudousers"].'" AND statutlivrai_panierachat LIKE "%'.$_POST["searchmot"].'%" ORDER BY id_panierachat DESC LIMIT 0,100 ');
													while($donnees = $req->fetch())
													{	
														$libel = "<span style='font-size:15px;color: red;'><strong>".$donnees["qte_panierachat"]."</strong></span> ".$donnees["libelle_panierachat"]." de <strong>".$donnees["prixunit_panierachat"]."</strong> F ";
														?>
												...
												elseif(isset($_POST["searchfirst"]) AND $_POST["searchin"] == "libelles")
												{
													$req = $bdd->query('SELECT pseudo_panierachat, libelle_panierachat, prixunit_panierachat, qte_panierachat, montant_panierachat, statutachat_panierachat, statutlivrai_panierachat, DATE_FORMAT(dateachat_panierachat, "%d/%m/%Y") AS dateachat_panierachatfr FROM panierachat WHERE pseudo_panierachat = "'.$_SESSION["pseudousers"].'" AND libelle_panierachat LIKE "%'.$_POST["searchmot"].'%" ORDER BY id_panierachat DESC LIMIT 0,100 ');
													while($donnees = $req->fetch())
													{	
														$libel = "<span style='font-size:15px;color: red;'><strong>".$donnees["qte_panierachat"]."</strong></span> ".$donnees["libelle_panierachat"]." de <strong>".$donnees["prixunit_panierachat"]."</strong> F ";
														?>
												...
												{
													include 'paginationcommande.php';
												   /* On calcule le nombre total d'entrées de notre table 
													09.que l'on stocke dans $nb_entrees */
													$req = $bdd->query('SELECT COUNT(*) AS total FROM panierachat WHERE pseudo_panierachat = "'.$_SESSION["pseudousers"].'" ');
													...
													$req = $bdd->query('SELECT pseudo_panierachat, libelle_panierachat, prixunit_panierachat, qte_panierachat, montant_panierachat, statutachat_panierachat, statutlivrai_panierachat, DATE_FORMAT(dateachat_panierachat, "%d/%m/%Y") AS dateachat_panierachatfr FROM panierachat WHERE pseudo_panierachat = "'.$_SESSION["pseudousers"].'" ORDER BY id_panierachat DESC LIMIT '.$start.', '.$entrees_par_page.' ');
													while($donnees = $req->fetch())
													{	
														$libel = "<span style='font-size:15px;color: red;'><strong>".$donnees["qte_panierachat"]."</strong></span> ".$donnees["libelle_panierachat"]." de <strong>".$donnees["prixunit_panierachat"]."</strong> F ";	
														?>
