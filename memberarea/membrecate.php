...
LIGAN O. F. Patrick ( +229 67 44 62 18)
 ...
 -->
    ...
    <meta charset="utf-8" />
    <title>Membre de même catégorie | LIBERTIS CLUB</title>
									...
									<div class="club-content-layout">
										<div class="club-content-layout-row">
											...
											<div class="club-layout-cell layout-item-0" style="width: 66%;" >
												...
												<!-- même catégories -->
												<h3>Membres de même catégories </h3><hr /><br />
												<div>
													...
													<?php
													$req = $bdd->query('SELECT COUNT(pseudo) AS nbrepseudouser FROM users WHERE pseudo = "'.$_SESSION["pseudousers"].'" AND statutmembre = "'.$_SESSION["statutmembre"].'" ');
													$donnees = $req->fetch();
													
													$nbreuser = $donnees["nbrepseudouser"];
													
													$req->closeCursor();
														
													if($nbreuser == 0)
													{
														echo "<div class='club-echec'>Il n'y a pas encore de membres de la même catégorie que vous pour le moment. </div>";
													}
													elseif(isset($_POST["search"]))
													{
														$req = $bdd->query("SELECT pseudo,nom,prenom,age,nationalite,Profession,statutmembre,activitephysique,religion,hobby,photo,affichephoto FROM users WHERE pseudo NOT IN ( SELECT pseudo FROM users WHERE pseudo = '".$_SESSION["pseudousers"]."') AND statutmembre = '".$_SESSION["statutmembre"]."' AND pseudo LIKE '%".$_POST["searchmembre"]."%' OR pseudo NOT IN ( SELECT pseudo FROM users WHERE pseudo = '".$_SESSION["pseudousers"]."') AND statutmembre = '".$_SESSION["statutmembre"]."' AND nom LIKE '%".$_POST["searchmembre"]."%' OR pseudo NOT IN ( SELECT pseudo FROM users WHERE pseudo = '".$_SESSION["pseudousers"]."') AND statutmembre = '".$_SESSION["statutmembre"]."' AND prenom LIKE '%".$_POST["searchmembre"]."%' ORDER BY pseudo ASC LIMIT 0,100 ");
													...
													<table>
														...
														<?php
														   ...
														   /* On calcule le nombre total d'entrées de notre table 
															...
															$req = $bdd->query("SELECT COUNT(*) AS total FROM users WHERE pseudo NOT IN ( SELECT pseudo FROM users WHERE pseudo = '".$_SESSION["pseudousers"]."') AND statutmembre = '".$_SESSION["statutmembre"]."' ");
															...
															$req = $bdd->query('SELECT pseudo,nom,prenom,age,nationalite,Profession,statutmembre,activitephysique,religion,hobby,photo,affichephoto FROM users WHERE pseudo NOT IN ( SELECT pseudo FROM users WHERE pseudo = "'.$_SESSION["pseudousers"].'") AND statutmembre = "'.$_SESSION["statutmembre"].'" ORDER BY pseudo ASC LIMIT '.$start.', '.$entrees_par_page.' ');
															while($donnees = $req->fetch())
															{
															?>
																<tr>
