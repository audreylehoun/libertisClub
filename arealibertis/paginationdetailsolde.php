...
function pagination($total,$courante)
{
		...
		// il y a au moins deux pages
		{
				...
				if ($total < 7 + ($adjacentes * 2))
				{
						...
						066.On la traite séparément pour avoir detailsolde.php
						067.au lieu de detailsolde.php?page=1
						068.et ainsi éviter le duplicate content
						069.cette ligne équivaut à :
						070.if ($courante == 1)
						071.$pagination.= "<span class=\"courante\">1</span>";
						072.else
						073.$pagination.= "<a href=\"detailsolde.php\">1</a>";
						074.*/
						$pagination.= ($courante == 1) ? "<span class=\"
						courante\">1</span>" :
						"<a href=\"detailsolde.php\">1</a>";
						...
						$pagination.= "<a href=\"detailsolde.php?page=$compteur\">
						$compteur</a>";
						}
				}
				...
				elseif($total > 5 + ($adjacentes * 2))
				{
						...
						if($courante < 1 + ($adjacentes * 2))
						{
							...
							// puis les huit pages suivantes
								...
								$pagination.= "<a href=\"detailsolde.php?page=$compteur\">$compteur</a>";
								}
							// les ... pour marquer la troncature
							//$pagination.= " ... ";
							// et enfin les deux dernières pages
							$pagination.= "<a href=\"detailsolde.php?page=$avder\">$avder</a>";
							$pagination.= "<a href=\"detailsolde.php?page=$total\">$total</a>";
						}
						/*
						...
						elseif($total - ($adjacentes * 2) > $courante && $courante > ($adjacentes * 2))
						{
								...
								$pagination.= "<a href=\"detailsolde.php\">1</a>";
								$pagination.= "<a href=\"detailsolde.php?page=2\">2</a>";
								// les ... pour marquer la troncature
								// $pagination.= " ... ";
								/* puis sept pages :
				...
				$pagination.= "<a href=\"detailsolde.php?page=$suiv\">suiv »</a>\n";
				else
